...
def reset ():
    global running,cycle_count,simuls
    running     = False
    cycle_count = 0
    simuls       = set()
...
def mouse_click(x,y):
    ...
    # _remove=[] #for multiple simultons in the same spot 
    # for s in simuls:
    #     if s.contains((x,y)):
    #         _remove.append(s)
    #
    ...
    #
        ...
        add(eval(name)(x,y,eval(name).radius*2,eval(name).radius*2))
...
def find(p):
    ...
    satisfied=set()
    for s in simuls:
        if p((s.get_location())) and issubclass(type(s),Prey):
            satisfied.add(s)
    #simuls=simuls-satisfied
    return satisfied
...
def display_all():
    ...
    controller.the_progress.config(text=str(cycle_count)+" updates/"+str(len(simuls))+" simultons")
